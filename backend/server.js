  ...
  if (app[method]) {
    ...
    app[method] = function(path, ...handlers) {
      ...
      try {
        ...
        console.error(`❌ Error registering ${method.toUpperCase()} route "${path}":`, error.message);
...
// Middleware
app.use(cors({
  origin: process.env.FRONTEND_URL || ['http://localhost:5173', 'https://your-frontend-domain.com'],
  credentials: true
}));
...
app.get('/api/db-status', async (req, res) => {
  try {
    ...
    const projectCount = await db.collection('projects')?.countDocuments() || 0;
